...
`.mofu` ファイルが検出されてから、実行可能なWasmへと昇華されるまでの全内部プロセスです。
...
インラインのRustロジックと、外部Goモジュールのダイレクトインポート、および子コンポーネントが1つの画面内で美しく共存する例です。
